...
function SharedSpaceList() {
  ...
  const { fetchSharedItems, reloadShared, setReloadShared, setLoading, setSharedcodeSpace, setSharedwebSpace, sharedcodeSpace, sharedwebSpace } = useData();
  ...
  const filterData = () => {
    ...
    if (current == "CodeSpace") {
      let filtered = sharedcodeSpace;
      filtered = filtered.filter(
        (space) =>
          space.heading?.toLowerCase().includes(prompt.toLowerCase()) ||
          space.language?.toLowerCase().includes(prompt.toLowerCase()) ||
          space.userid?.toLowerCase().includes(prompt.toLowerCase())
      );
      setFilteredcodeData(filtered);
    }

    if (current == "WebSpace") {
      let filtered = sharedwebSpace;
      filtered = filtered.filter(
        (space) =>
          space.heading?.toLowerCase().includes(prompt.toLowerCase()) ||
  ...
  return (
    <div className="w-full bg-white h-full px-3 py-2 rounded-lg">
      <Tabs.Root className="flex flex-col w-full h-full overflow-y-auto" defaultValue="CodeSpace">
        ...
        <Tabs.Content value="CodeSpace" className={`rounded-md focus:outline-none pb-1 overflow-y-auto grid ${(sharedcodeSpace.length > 0 && filteredcodeData.length > 0) ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'} gap-6 w-full px-3 md:px-5`}>
          {filteredcodeData.length > 0 && filteredcodeData.map((space, index) => (
            <div key={space.spaceid} className="relative">
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  <Link to={space.isEditorMode ? `/dashboard/shared/codespace/edit/${space.spaceid}` : `/dashboard/shared/codespace/view/${space.spaceid}`} className="flex items-center">
                    <img
          ...
          {(sharedcodeSpace.length == 0 && !(filteredcodeData.length == 0)) && <div className="flex flex-col items-center justify-center h-[70vh]">
            <img
              src="https://ik.imagekit.io/vituepzjm/undraw_void_-3-ggu.svg?updatedAt=1733491948383"
              alt="Nothing to show"
              className="w-40 mb-6"
            />
            <h2 className="text-base break-words italic text-gray-500">No WebSpaces are shared with you.</h2>
          </div>}
          {(filteredcodeData.length == 0 && !(sharedcodeSpace.length == 0)) && <div className="flex flex-col items-center justify-center h-[70vh]">
            <img
              src="https://ik.imagekit.io/vituepzjm/undraw_void_-3-ggu.svg?updatedAt=1733491948383"
              alt="Nothing to show"
              className="w-40 mb-6"
            />
            <h2 className="text-base break-words italic text-gray-500">No CodeSpaces are found.</h2>
          </div>}
          {(filteredcodeData.length == 0 && sharedcodeSpace.length == 0) && <div className="flex flex-col items-center justify-center h-[70vh]">
            <img
              src="https://ik.imagekit.io/vituepzjm/undraw_void_-3-ggu.svg?updatedAt=1733491948383"
        ...
        <Tabs.Content value="WebSpace" className={`rounded-md focus:outline-none pb-1 overflow-y-auto grid ${(sharedwebSpace.length > 0 && filteredwebData.length > 0) ? 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-3' : 'grid-cols-1'} gap-6 w-full px-3 md:px-5`}>
          ...
          {(sharedwebSpace.length == 0 && !(filteredwebData.length == 0)) && <div className="flex flex-col items-center justify-center h-[70vh]">
            <img
              src="https://ik.imagekit.io/vituepzjm/undraw_void_-3-ggu.svg?updatedAt=1733491948383"
              alt="Nothing to show"
              className="w-40 mb-6"
          ...
          {(filteredwebData.length == 0 && !(sharedwebSpace.length == 0)) && <div className="flex flex-col items-center justify-center h-[70vh]">
            <img
              src="https://ik.imagekit.io/vituepzjm/undraw_void_-3-ggu.svg?updatedAt=1733491948383"
              alt="Nothing to show"
              className="w-40 mb-6"
            />
            <h2 className="text-base break-words italic text-gray-500">No WebSpaces are found.</h2>
          </div>}
          {(filteredwebData.length == 0 && sharedwebSpace.length == 0) && <div className="flex flex-col items-center justify-center h-[70vh]">
            <img
              src="https://ik.imagekit.io/vituepzjm/undraw_void_-3-ggu.svg?updatedAt=1733491948383"
