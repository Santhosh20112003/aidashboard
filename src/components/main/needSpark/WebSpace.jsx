...
import { RiFullscreenExitLine, RiRefreshLine, RiSettingsLine } from 'react-icons/ri';
import * as Dialog from "@radix-ui/react-dialog";
import * as Popover from "@radix-ui/react-popover";
import { MdIosShare } from 'react-icons/md';
import * as Avatar from "@radix-ui/react-avatar";
import * as Tooltip from "@radix-ui/react-tooltip";
import { Frameworks, FrameworksArray } from '../../../constants';
...
function WebSpace() {
    ...
    <div id="navbar"> <a id="toggle-data" href="#data"><i class="icon-data"></i>Data</a>
        <h1>3D Solar System<br></h1> <a id="toggle-controls" href="#controls"><i class="icon-controls"></i>Controls</a>
    </div>
    <div id="data"> <a class="sun" title="sun" href="#sunspeed">Sun</a> <a class="mercury" title="mercury"
            href="#mercuryspeed">Mercury</a> <a class="venus" title="venus" href="#venusspeed">Venus</a> <a
            class="earth active" title="earth" href="#earthspeed">Earth</a> <a class="mars" title="mars"
            href="#marsspeed">Mars</a> <a class="jupiter" title="jupiter" href="#jupiterspeed">Jupiter</a> <a
            class="saturn" title="saturn" href="#saturnspeed">Saturn</a> <a class="uranus" title="uranus"
            href="#uranusspeed">Uranus</a> <a class="neptune" title="neptune" href="#neptunespeed">Neptune</a> </div>
    <div id="controls"> <label class="set-view"> <input type="checkbox"> </label>
        <label class="set-zoom"> <input type="checkbox"> </label>
        <label> <input type="radio" class="set-speed" name="scale" checked> <span>Speed</span> </label>
        <label> <input type="radio" class="set-size" name="scale"> <span>Size</span> </label>
        <label> <input type="radio" class="set-distance" name="scale"> <span>Distance</span> </label>
    </div>
    ...
    <script type="text/javascript">
        if (typeof jQuery == 'undefined') { document.write(unescape("%3Cscript src='js/jquery.min.js' type='text/javascript'%3E%3C/script%3E")); }      
    </script>
...
/* Copyright (c) 2012 Julian Garnier Licensed under the MIT license */
html,
body,
div,
span,
...
/* --------------------------------------------------------------------------- saturn ring */
/* #saturn .ring { animation-name: shadow-saturn-ring; } @keyframes shadow-saturn-ring{ 0% { box-shadow: inset -16px 0 4px rgba(20, 40, 50, 0.45); opacity: .4; } 25% { box-shadow: inset -5px -30px 12px rgba(20, 40, 50, 0.3); opacity: 1; } 25.01% { box-shadow: inset 5px -30px 12px rgba(20, 40, 50, 0.3); opacity: 1; } 50% { box-shadow: inset 16px 0 4px rgba(20, 40, 50, 0.45); opacity: .4; } 75% { box-shadow: inset 0 30px 50px rgba(20, 40, 50, 0); opacity: .3; } 100% { box-shadow: inset -16px 0 4px rgba(20, 40, 50, 0.45); opacity: .4; } } .scaled.view-2D #saturn .ring, .scaled.view-3D #saturn .ring { box-shadow: inset 16px 0 4px rgba(20, 40, 50, 0.6); } */
/* --------------------------------------------------------------------------- uranus ; pos: Left */
#uranus .pos {
    left: 0;
    top: 50%;
...
/* --------------------------------------------------------------------------- DEBUG .pos, .planet, .orbit, .ring { animation-duration: 20s!important; } /* */
/* --------------------------------------------------------------------------- navigation styles */
dl.infos {
    position: absolute;
    display: block;
...
@media screen and (min-width: 1600px) {
    ...
    const { setTheme, theme, Loading } = useData();
    ...
    return (
        <div id="webspace" className="w-full h-[90vh] md:px-4 md:pb-2 px-2 pt-2 bg-white flex gap-4">
            <div className={`md:w-1/2 w-full h-full space-y-4`}>
                <div className={`h-[70vh] bg-gray-100 border editor-t border-gray-100 rounded-md`}>
                    ...
                    <div style={{
                        position: "relative",
                        overflow: "hidden",
                    }} className={`bg-gray-200 rounded-b-md h-[90%] `}>{renderEditor()}</div>
                </div>
                <div>
                    <div className="w-full max-h-[50px] flex gap-3 items-center">
                        <textarea value="A simple 2D representation of the solar system." className={`w-full input-t max-h-[50px] h-[8vh] px-4 py-2  border border-gray-300 rounded-lg resize-none focus:ring-2 focus:ring-black`} placeholder="Type your prompt here..." />
                        <Dialog.Root>
                            ...
                            <Dialog.Portal >
                                <Dialog.Overlay className="fixed inset-0 bg-black/30 z-[10000000000] data-[state=open]:animate-overlayShow" />
                                <Dialog.Content className="fixed left-1/2 overflow-y-auto z-[10000000000000] top-1/2 max-h-[85vh] w-[90vw] max-w-[800px] -translate-x-1/2 -translate-y-1/2 rounded-md bg-white p-[25px] shadow-[hsl(206_22%_7%_/_35%)_0px_10px_38px_-10px,_hsl(206_22%_7%_/_20%)_0px_10px_20px_-15px] focus:outline-none data-[state=open]:animate-contentShow">
                                    <div className="flex items-center justify-between">
                                        <div className="m-0 text-md font-bold md:text-xl text-center md:text-start mb-3 text-gray-800">
                                            A simple 2D representation of the solar system.
                                        </div>
                                        <button>
                                            <BiSolidCopy className="text-gray-600 text-sm" />
                                        </button>
                                    </div>
                                    <hr className="mb-3 bg-gray-600" />
                                    <div className="text-gray-600 jarvis no-tailwindcss text-sm md:text-base" dangerouslySetInnerHTML={{ __html: converter.makeHtml('This code creates a basic visual representation of the solar system using HTML for structure, CSS for styling and positioning, and JavaScript for any dynamic behaviors (currently a placeholder). Planets orbit the sun in a circular path. The Earth has a moon orbiting it. Saturn has rings. The sizes and orbital speeds are simplified for visual clarity. More advanced features would require a JavaScript framework or library for 3D rendering') }} />
                                </Dialog.Content>
                            </Dialog.Portal>
                        </Dialog.Root>
                    </div>
                    <div className="flex button-t gap-3 mt-2 max-h-[10vh] items-center">
                        <button
                            className={`w-full py-4 active:scale-[99%] transition-all flex items-center justify-center gap-2 bg-black text-white font-semibold rounded-lg hover:bg-black`}
                ...
                <div className="h-full preview-t w-full bg-gray-50 border rounded shadow flex flex-col">
                    <div className="px-2 py-2 flex w-full items-center justify-between gap-2 border-b bg-gray-100 text-gray-600 font-semibold">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <button className="bg-gray-200 focusmode-t p-2 rounded-md active:scale-90 transition-all">
                                <RiFullscreenExitLine className="text-lg text-gray-700" />
                            </button>
                            <button className="bg-gray-200 share-t p-2 rounded-md active:scale-90 transition-all">
                                <MdIosShare />
                            </button>
                        </div>
                    </div>
